...
<script setup lang="ts">
  ...
  import ChannelSearch from '../payInChannel/modules/channel-search.vue'
  import ChannelDialog from '../payInChannel/modules/channel-dialog.vue'
  import {
    ElCard,
    ElSpace,
    ElMessageBox,
  ...
  defineOptions({ name: 'PayOutChannel' })
  ...
  const fetchChannelMerchantMap = async () => {
    try {
      const res = await getChannelMerchantInfoList({ type: 2 })
  ...
  const fetchProductMap = async () => {
    try {
      const res = await getProductMap({ type: 2 }) // type: 2 代付
      const map = new Map<number, string>()
      ;(res.pageData || []).forEach((item) => {
        map.set(item.id, item.name)
      })
  ...
  const fetchAgentMap = async () => {
    try {
      const res = await getAgentMap({ type: 2 }) // type: 2 代付
      const map = new Map<number, string>()
      ;(res.pageData || []).forEach((item) => {
        map.set(item.id, item.name)
      })
